...
async function finalizePaidUnlocked(record,{source='redsys',sendCustomerEmail=true}={}){
 ...
 try{
  ...
  if(sendCustomerEmail)await notifyCustomer(record).catch(()=>false);
  await webhooks.emit('order.paid',{order:record.order,status:record.status,amount:record.amount,currency:record.currency,fulfillment:record.fulfillment||'shipping'}).catch(()=>{});
...
async function finalizeFailed(record){
 ...
 return withLock(`finalize:${record.order}`,async()=>{
  ...
  await webhooks.emit('order.payment_failed',{order:record.order,status:record.status,amount:record.amount,currency:record.currency}).catch(()=>{});
  return{ok:true};
 },{ttlMs:5*60*1000});
}
module.exports={createFulfillment,createReconciliation,notifyCustomer,finalizePaid,finalizeFailed,_test:{finalizePaidUnlocked,finalizedMarker}};
