...
function buildSitemap({baseUrl='https://nutretium.com',products=[],categories=[],intents=SEO_INTENTS}={}){
  ...
  for(const category of categories||[]){
    if(!String(category||'').trim()||!activeProducts.some(product=>product.category===category))continue;
  ...
  for(const product of activeProducts){
    urls.push({loc:`${base}${productPath(product)}`,changefreq:'weekly',priority:product.featured?'0.8':'0.7'});
  }
