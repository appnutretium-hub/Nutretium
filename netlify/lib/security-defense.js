...
function expectedOrigins() {
  const values = [process.env.ALLOWED_ORIGIN, process.env.URL, process.env.DEPLOY_PRIME_URL]
...
function requestFingerprint(event) {
  const ua = header(event, 'user-agent').trim().replace(/\s+/g, ' ').slice(0, 512);
  const lang = header(event, 'accept-language').trim().replace(/\s+/g, ' ').slice(0, 128);
  const platform = header(event, 'sec-ch-ua-platform').trim().slice(0, 128);
...
function assertBrowserBoundary(event) {
  ...
  if (event?.body && (!contentType || !contentType.includes('application/json'))) {
    throw Object.assign(new Error('Content-Type no permitido.'), { code: 'CONTENT_TYPE_BLOCKED' });
  }
  const size = Buffer.byteLength(String(event?.body || ''), 'utf8');
  if (size > 5 * 1024 * 1024) {
    throw Object.assign(new Error('Solicitud demasiado grande.'), { code: 'BODY_TOO_LARGE' });
  }
  return true;
}
...
function assertSessionBinding(event, claims) {
  ...
  const actual = requestFingerprint(event);
  const expected = String(claims.fp || '');
  const a = Buffer.from(actual), b = Buffer.from(expected);
  if (a.length !== b.length || !crypto.timingSafeEqual(a, b)) {
    throw Object.assign(new Error('La sesión cambió de contexto.'), { code: 'SESSION_CONTEXT_CHANGED' });
  }
  return true;
}

function requestId(event) {
  return header(event, 'x-nutretium-request').trim();
}
...
async function consumeMutationNonce({ event, actor, scope }) {
  ...
  if (!validRequestId(id)) return { ok: false, statusCode: 400, code: 'REQUEST_ID_REQUIRED', error: 'Identificador seguro de operación ausente o inválido.' };
  const day = new Date().toISOString().slice(0, 10).replace(/-/g, '');
  const store = getBlobStore(`security-replay-${day}`);
  ...
  let result;
  try {
    result = await store.setJSON(key, { at: new Date().toISOString(), actor: String(actor || '').toLowerCase(), scope: String(scope || '') }, { onlyIfNew: true });
  } catch {
    return { ok: false, statusCode: 503, code: 'REPLAY_GUARD_UNAVAILABLE', error: 'El control anti-replay no está disponible.' };
  }
