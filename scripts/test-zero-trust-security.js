...
try{
 ...
 assert.equal(policy.productionLike(),true);assert.equal(policy.staffMfaRequired(),true,'MFA debe ser obligatorio en producción');assert.equal(policy.staffCookieRequired(),true);assert.equal(policy.staffCsrfRequired(),true);assert.equal(policy.staffStepUpRequired(),true);
 ...
 const loginSource=read('netlify/functions/staff-login.js');assert(loginSource.includes('STAFF_LOGIN_TTL_SECONDS'));assert(loginSource.includes('mfaReplay.consume'));assert(loginSource.includes('newSessionBinding'));assert(!loginSource.includes('privilegedMfaExempt'),'ningún rol privilegiado puede quedar exento de MFA');assert(loginSource.includes("normalized==='owner'||normalized==='admin'"));assert(loginSource.includes('user?.mfaEnabled===true'));
 const login=require('../netlify/functions/staff-login')._test;assert.equal(login.mfaRequiredFor('owner',{}),true,'owner debe requerir MFA');assert.equal(login.mfaRequiredFor('admin',{}),true,'admin debe requerir MFA');assert.equal(login.mfaRequiredFor('staff',{}),true,'producción debe requerir MFA para staff');
 ...
 const stepup=read('netlify/lib/security-step-up.js');assert(stepup.includes("claims.sid!==auth.claims.sid"));assert(!stepup.includes('privileged-mfa-exempt'),'Owner/Admin no pueden eludir el step-up MFA');
 ...
 console.log('[zero-trust-security] OK · MFA privilegiado + cookie HttpOnly + CSRF + session binding + anti-replay obligatorios en producción');
