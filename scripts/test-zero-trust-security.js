...
try{
 process.env.CONTEXT='production';process.env.REQUIRE_STAFF_MFA='false';process.env.REQUIRE_STAFF_COOKIE='false';process.env.REQUIRE_STAFF_CSRF='false';process.env.REQUIRE_STAFF_STEP_UP='false';
 assert.equal(policy.productionLike(),true);assert.equal(policy.staffMfaRequired(),true);assert.equal(policy.staffCookieRequired(),true);assert.equal(policy.staffCsrfRequired(),true);assert.equal(policy.staffStepUpRequired(),true);
 ...
 const login=read('netlify/functions/staff-login.js');assert(login.includes('STAFF_LOGIN_TTL_SECONDS'));assert(login.includes('mfaReplay.consume'));assert(login.includes('newSessionBinding'));
 ...
 const stepup=read('netlify/lib/security-step-up.js');assert(stepup.includes("claims.sid!==auth.claims.sid"));
 for(const file of['netlify/functions/admin-settings.js','netlify/functions/admin-governance.js','netlify/functions/platform-restore.js','netlify/functions/refund-redsys.js']){const source=read(file);assert(source.includes('verifyStepUp'),`${file} must enforce privileged step-up`);assert(source.includes('consumeMutationNonce'),`${file} must enforce anti-replay on critical mutation`)}
