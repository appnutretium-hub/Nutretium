...
async function run({view,requestedBy='scheduled-ai-worker'}={}){
  ...
  const content=JSON.stringify({validation:result.validation,coveragePct:result.coveragePct,risks:result.risks,recommendations:result.recommendations});
  await memory.remember({
    type:'learning',
    agent:SYSTEM_AGENT,
    subject:'daily-growth-loop',
    content,
    evidence:result.evidence,
    confidence:result.validation==='PARCIALMENTE_VALIDADO'?'medium':'low',
    outcome:'recommendations-only'
  }).catch(()=>{});
