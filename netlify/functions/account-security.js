...
function revokeAllSessions(user){user.tokensValidAfter=Math.floor(Date.now()/1000);user.passwordChangedAt=new Date().toISOString();return user}
exports.handler=async function(event){
 ...
 if(body.action==='change-password'){
  ...
  user.passwordHash=hashPassword(next);revokeAllSessions(user);await usuarios.escribe(email,user);return json(200,{ok:true,reauthRequired:true,message:'Contraseña actualizada. Por seguridad, vuelve a iniciar sesión.'});
 }
 ...
 if(body.action==='reset-password'){
  ...
  user.passwordHash=hashPassword(next);revokeAllSessions(user);await usuarios.escribe(record.email,user);record.used=true;record.usedAt=new Date().toISOString();await store.setJSON(key,record);return json(200,{ok:true,message:'Contraseña actualizada. Ya puedes iniciar sesión.'});
 }
