...
exports.handler=async event=>{
  ...
  try{
    ...
    const task=String(body.task||'cross-check');
    const view=await runtime.businessSnapshot();
    const learned=await memory.learningContext({subject:task,agent:'ai_federation',limit:6});
    const context={...view,operations:{...(view.operations||{}),learning:learned}};
    const result=await federation.query({question:body.question,task,context,minModels:body.minModels||2,maxModels:body.maxModels||3,learn:body.learn!==false});
    await audit.append({event,actor:auth.email,action:'AI_FEDERATION_QUERY',resource:task,outcome:'SUCCESS',metadata:{status:result.status,providers:result.results?.map(x=>x.provider)||[],learningContext:learned.length,externalSpendLimitEur:0}}).catch(()=>{});
