...
function assertStaffCsrf(event,claims){if(!mutatingMethod(event?.httpMethod)||!staffCsrfRequired())return true;const headers=event?.headers||{},supplied=headers['x-nutretium-csrf']||headers['X-Nutretium-CSRF'];if(!safeEqual(supplied,claims?.csrf))throw Object.assign(new Error('CSRF inválido'),{code:'CSRF_INVALID'});return true}
function securityHeaders(extra={}){const headers={'Cache-Control':'no-store','Pragma':'no-cache','X-Content-Type-Options':'nosniff','X-Frame-Options':'DENY','Referrer-Policy':'no-referrer','X-DNS-Prefetch-Control':'off','X-Permitted-Cross-Domain-Policies':'none','Cross-Origin-Opener-Policy':'same-origin','Cross-Origin-Resource-Policy':'same-site','Origin-Agent-Cluster':'?1','Permissions-Policy':'camera=(), microphone=(), geolocation=(), payment=(), usb=(), serial=(), bluetooth=()',...extra};if(productionLike())headers['Strict-Transport-Security']='max-age=63072000; includeSubDomains';return headers}
module.exports={STAFF_LOGIN_TTL_SECONDS,STAFF_SESSION_TTL_SECONDS,STEP_UP_TTL_SECONDS,STAFF_IDLE_REAUTH_SECONDS,productionLike,staffMfaRequired,staffCsrfRequired,staffCookieRequired,staffStepUpRequired,randomToken,safeEqual,mutatingMethod,assertStaffCsrf,securityHeaders};
