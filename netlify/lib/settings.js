...
const defaults={
 ...
 shipping:{managed:false,enabled:false,rateCents:null,freeFromCents:5000,country:'España',label:'Envío'},
...
function safeUrl(v){const raw=String(v||'').trim().slice(0,300);if(!raw)return'';try{const u=new URL(raw);return u.protocol==='https:'?u.toString():''}catch{return''}}
...
function normalize(v){
 ...
 return{
  ...
  contact:{phone:String(contact.phone||defaults.contact.phone).replace(/[^0-9+]/g,'').slice(0,20)||defaults.contact.phone,email:safeText(contact.email,defaults.contact.email,120).toLowerCase(),hours:safeText(contact.hours,defaults.contact.hours,100),address:safeText(contact.address,defaults.contact.address,180)},
...
module.exports={read,readWithVersion,write,normalize,versionOf,defaults,normalizeNav};
