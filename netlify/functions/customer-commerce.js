...
const actor=(email)=>({email,role:'client'});
...
function purchased(order,productId){return Array.isArray(order?.items)&&order.items.some(i=>Number(i.id)===Number(productId));}
function referralCode(){return crypto.randomBytes(6).toString('base64url').toUpperCase().replace(/[^A-Z0-9]/g,'').slice(0,10);}
exports.handler=async function(event){
  if(event.httpMethod==='OPTIONS')return{statusCode:204,headers:CORS,body:''};
  if(event.httpMethod!=='POST')return response(405,{error:'Method Not Allowed'});
  const who=auth(event);if(!who.ok)return response(who.statusCode,{error:who.error});
  let body;try{body=JSON.parse(event.body||'{}');}catch{return response(400,{error:'JSON no válido.'});}
  const action=String(body.action||'');
  try{
    if(action==='overview'){
      const domains=['returns','crm-tickets','subscriptions','saved-carts','privacy-requests','loyalty','reviews','referrals','consent-settings'];
      const [returns,tickets,subs,carts,privacy,loyalty,reviews,referrals,consents]=await Promise.all(domains.map(d=>store.list(d,{limit:500})));
      const own=(rows)=>rows.filter(r=>String(r.customerEmail||r.ownerEmail||'').toLowerCase()===who.email);
      return response(200,{returns:own(returns),tickets:own(tickets),subscriptions:own(subs),savedCarts:own(carts),privacyRequests:own(privacy),loyalty:own(loyalty)[0]||null,reviews:own(reviews),referrals:own(referrals),consents:own(consents)});
    }
    if(action==='save-cart'){
    ...
    if(action==='archive-cart'){
      const previous=await store.get('saved-carts',body.id);if(!previous||previous.customerEmail!==who.email)return response(404,{error:'Carrito no encontrado.'});return response(200,{record:await store.archive('saved-carts',body.id,actor(who.email),'customer-archive')});
    }
    ...
    if(action==='submit-review'){
      const order=await ownOrder(who.email,body.orderId);if(!order||order.status!=='PAID')return response(403,{error:'La reseña requiere una compra pagada de tu cuenta.'});const product=(NUTRETIUM_PRODUCTS||[]).find(p=>Number(p.id)===Number(body.productId));if(!product||!purchased(order,product.id))return response(403,{error:'Ese producto no figura en el pedido indicado.'});const rating=Number(body.rating);const id=`${order.order}:${product.id}`;const checked=schema.validate('reviews',{id,productId:product.id,sku:product.code,orderId:order.order,customerEmail:who.email,rating,title:String(body.title||'').slice(0,120),body:String(body.body||'').slice(0,2000),verifiedPurchase:true,status:'pending'});if(!checked.ok)return response(400,{error:checked.errors[0]});return response(201,{record:await store.save('reviews',checked.data,actor(who.email),{id,reason:'verified-review'})});
    }
    ...
    if(action==='consent-set'){
      const purpose=String(body.purpose||'');if(!['analytics','marketing','personalization'].includes(purpose))return response(400,{error:'Finalidad no válida.'});const id=`${who.email}:${purpose}`;const checked=schema.validate('consent-settings',{id,customerEmail:who.email,purpose,status:body.granted===true?'granted':'withdrawn',source:'account',changedAt:new Date().toISOString()});return response(200,{record:await store.save('consent-settings',checked.data,actor(who.email),{id,reason:'customer-consent'})});
    }
    ...
    if(action==='referral-create'){
      const current=(await store.list('referrals',{limit:500})).find(r=>r.ownerEmail===who.email&&r.status==='active');if(current)return response(200,{record:current});let code=referralCode();for(let i=0;i<5;i++){const exists=(await store.list('referrals',{limit:500})).some(r=>r.code===code);if(!exists)break;code=referralCode();}const id=crypto.randomUUID();const checked=schema.validate('referrals',{id,code,ownerEmail:who.email,status:'active',conversions:0});return response(201,{record:await store.save('referrals',checked.data,actor(who.email),{id,reason:'customer-referral'})});
    }
    if(action==='gift-card-balance'){
      const code=String(body.code||'').trim().toUpperCase();if(!code)return response(400,{error:'Falta el código.'});const cards=await store.list('gift-cards',{limit:1000});const card=cards.find(c=>String(c.code||'').toUpperCase()===code);if(!card)return response(404,{error:'Tarjeta no encontrada.'});return response(200,{giftCard:{status:card.status,currency:card.currency,balanceCents:Number(card.balanceCents??card.initialCents??0)}});
    }
    if(action==='b2b-apply'){
