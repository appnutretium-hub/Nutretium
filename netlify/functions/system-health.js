...
exports.handler=async function(event){
 ...
 const checks={
  ...
  redsysSecret:set('REDSYS_SECRET_KEY'),redsysMerchant:set('REDSYS_MERCHANT_CODE'),redsysProduction:process.env.REDSYS_ENV==='production',
  commerceLive:process.env.COMMERCE_LIVE==='true',shipping:shipping.configured(),
 ...
 const critical=['jwt','admin','blobs','redsysSecret','redsysMerchant','redsysProduction','commerceLive','shipping','emailProvider','emailRecipient','emailFrom'];
 const ready=critical.every(k=>checks[k]);
 const missing=critical.filter(k=>!checks[k]);
 return{statusCode:200,headers:CORS,body:JSON.stringify({ready,environment:process.env.REDSYS_ENV||'test',checks,missing})};
};
