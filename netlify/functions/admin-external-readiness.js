...
exports.handler=async function(event){
 ...
 try{
  const [commerce,payment,dependencies]=await Promise.all([settings.read(),paymentConfig.resolve(),assessCommerceDependencies()]);
  ...
  return reply(200,await assessExternalReadiness({env,shipping:commerce.shipping,factusol:dependencies.factusol,catalog:dependencies.catalog,compliance:dependencies.compliance}));
 }catch(error){
