...
const enabled=env=>String(env.AI_OLLAMA_QUEUE_ENABLED||'false').toLowerCase()==='true';
function config(env=process.env){
 const on=enabled(env),model=String(env.AI_OLLAMA_MODEL||'').trim();
 if(on&&!model)throw new Error('AI_OLLAMA_MODEL es obligatorio cuando la cola Ollama está activa.');
 return{enabled:on,model:model||null,externalSpendLimitEur:0,transport:'outbound_runner'};
}
...
async function enqueueOne({agent,action,view,validation='NO_VALIDADO',env=process.env}){
 const cfg=config(env);if(!cfg.enabled)return{enabled:false,created:false,job:null};
...
async function enqueuePortfolio({runs=runtime.PORTFOLIO_RUNS,env=process.env}={}){
 const cfg=config(env);if(!cfg.enabled)return{enabled:false,queued:0,reused:0,jobs:[]};
...
async function status(env=process.env){
 const cfg=config(env);if(!cfg.enabled)return{...cfg,queue:{pending:0,processing:0,completed:0,failed:0,total:0,lastCompletedAt:null,lastModel:null}};
 return{...cfg,queue:await queue.stats()};
}
module.exports={SYSTEM_PROMPT,enabled,config,fingerprint,messages,enqueueOne,enqueuePortfolio,status};
