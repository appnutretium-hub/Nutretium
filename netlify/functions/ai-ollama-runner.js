...
async function authorised(event,runnerId){
 const token=tokenFrom(event),expected=process.env.AI_OLLAMA_RUNNER_TOKEN||'';
 if(expected.length>=24&&sameSecret(token,expected))return true;
 return pairing.authenticate({token,runnerId});
}
...
exports.handler=async function(event){
 ...
 try{
  if(action==='pair'){
   if(!runnerId||!body.code)return json(400,{error:'runnerId y code son obligatorios.'});
   const result=await pairing.consumePairing({code:body.code,runnerId});
   return json(result.ok?200:401,result.ok?{ok:true,runnerId:result.runnerId,token:result.token,model:result.config.model,enabled:result.config.enabled}:{ok:false,code:result.code});
  }
  if(!await authorised(event,runnerId))return json(401,{error:'Runner no autorizado.'});
  if(action==='claim'){
  ...
  if(action==='status')return json(200,{ok:true,queue:await queue.stats(),pairing:await pairing.status()});
