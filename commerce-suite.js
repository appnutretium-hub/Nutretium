...
function crossSell(){
  ...
  const box=document.createElement('div');box.id='suiteCrossSell';box.style.cssText='margin:14px 0;padding:14px;border:1px solid #2a2a2a;border-radius:14px;background:#101010';box.innerHTML='<p style="font-size:12px;font-weight:800;color:#d4af37;margin:0 0 10px">COMPLETA TU PEDIDO</p>'+candidates.map(p=>`<button data-suite-add="${p.id}" style="width:100%;display:flex;justify-content:space-between;background:none;color:#eee;border:0;padding:7px 0;cursor:pointer;text-align:left"><span>${String(p.name).replace(/[<>]/g,'')}</span><b>${Number(p.price).toFixed(2)} €</b></button>`).join('');target.parentElement?.insertBefore(box,target);box.querySelectorAll('[data-suite-add]').forEach(b=>b.onclick=()=>window.addToCart(Number(b.dataset.suiteAdd)));
}
...
document.addEventListener('DOMContentLoaded',()=>{trustBar();addNavigation();crossSell();if(location.search.includes('cart=1')&&typeof openCart==='function')setTimeout(openCart,200)});
})();
