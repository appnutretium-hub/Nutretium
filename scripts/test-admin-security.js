...
const ownerWithoutMfa=signJWT({sub:user.id,email,kind:'staff-login',mfa:false,exp:Math.floor(Date.now()/1000)+3600});
...
(async()=>{
 ...
 const ownerExchange=await adminSession.handler(event({action:'exchange'},{authorization:`Bearer ${ownerWithoutMfa}`}));
 ...
 const ownerCookie=ownerExchange.headers['Set-Cookie'];
 assert(ownerCookie&&/nt_staff_session=/.test(ownerCookie),'Debe emitirse la cookie interna');
 assert(/HttpOnly/i.test(ownerCookie),'La cookie debe ser HttpOnly');
 assert(/Secure/i.test(ownerCookie),'La cookie debe ser Secure');
 assert(/SameSite=Strict/i.test(ownerCookie),'La cookie debe ser SameSite=Strict');
 const cookiePair=ownerCookie.split(';')[0];
 const status=await adminSession.handler(event({action:'status'},{cookie:cookiePair}));
 assert.strictEqual(status.statusCode,200,'La cookie owner debe autenticar la sesión interna');
 assert.strictEqual(parse(status).email,email);
 assert.strictEqual(parse(status).mfa,false);

 const exchange=await adminSession.handler(event({action:'exchange'},{authorization:`Bearer ${staffBearer}`}));
 assert.strictEqual(exchange.statusCode,200,'Owner con MFA también debe seguir siendo compatible');
 ...
 assert.strictEqual(writes.length,12);assert(writes.every(row=>row.metadata.token==='[REDACTED]'),'El audit log debe censurar tokens');
 ...
 assert(adminHtml.includes('/staff-session-bridge.js'),'El build debe inyectar el puente en admin');assert(backofficeHtml.includes('/staff-session-bridge.js'),'El build debe inyectar el puente en backoffice');
 ...
 console.log('[test-admin-security] OK · owner password-only · MFA admin · cookie HttpOnly · auditoría íntegra');
