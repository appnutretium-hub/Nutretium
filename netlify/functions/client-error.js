...
exports.handler=async function(event){
 ...
 // La telemetría es prescindible: si el limitador no puede demostrar que la
 // petición está permitida, se descarta el evento en vez de aceptar tráfico
 // ilimitado. Respondemos 204 para no afectar la UX del cliente.
 const rate=await consume({scope:'client-error',event,limit:20,windowMs:10*60*1000}).catch(()=>({allowed:false,degraded:true}));
