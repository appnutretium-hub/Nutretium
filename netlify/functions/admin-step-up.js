...
exports.handler=async event=>{
 ...
 const now=Math.floor(Date.now()/1000),token=signJWT({sub:auth.user.id,email:auth.email,kind:'staff-step-up',mfa:true,sv:Number(auth.user.sessionVersion||0),exp:now+security.STEP_UP_TTL_SECONDS});
