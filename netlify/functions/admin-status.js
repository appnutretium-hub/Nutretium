...
exports.handler=async function(event){
  ...
  const services={
    ...
    blobs:yes('SITE_ID','NETLIFY_API_TOKEN'),catalogPublishing:yes('GITHUB_TOKEN'),
    fraudVelocity:!process.env.FRAUD_MAX_CHECKOUTS_15M||yes('SITE_ID','NETLIFY_API_TOKEN'),
  };
  const capabilities={
    catalog:{status:'operational',requires:[]},
    orders:{status:services.blobs?'operational':'blocked',requires:services.blobs?[]:['SITE_ID','NETLIFY_API_TOKEN']},
    enterprisePlatform:{status:services.blobs?'operational':'blocked',requires:services.blobs?[]:['SITE_ID','NETLIFY_API_TOKEN']},
    ...
    backups:{status:services.blobs?'operational':'blocked',requires:services.blobs?[]:['SITE_ID','NETLIFY_API_TOKEN']},
  ...
  if(!services.blobs)warnings.push('Faltan credenciales explícitas de Netlify Blobs para persistencia empresarial.');
