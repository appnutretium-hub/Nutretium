...
exports.handler=async function(event){
  ...
  if(body.token){
    try{
      const email=verifyJWT(body.token).email;
      const comprador=await usuarios.lee(email);
      if(comprador){const envio=direccion.normaliza(comprador.direccion),problem=direccion.revisa(envio);if(problem)return json(422,{error:problem,motivo:'sin-direccion'});identidad={email,comprador,envio,guest:false}}
    }catch{}
  }
