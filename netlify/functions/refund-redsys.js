...
function verifyRefundResponse(data,{secret,orderId,amountCents,merchant,terminal}){
 ...
 const code=String(field(decoded,'Ds_Response','DS_RESPONSE')||''),order=String(field(decoded,'Ds_Order','DS_ORDER')||''),amount=Number(field(decoded,'Ds_Amount','DS_AMOUNT')),currency=String(field(decoded,'Ds_Currency','DS_CURRENCY')||''),tx=String(field(decoded,'Ds_TransactionType','DS_TRANSACTIONTYPE')||''),responseMerchant=String(field(decoded,'Ds_MerchantCode','DS_MERCHANTCODE')||''),responseTerminal=String(field(decoded,'Ds_Terminal','DS_TERMINAL')||'');
 if(order!==String(orderId))return{ok:false,error:'Redsys devolvió otro número de pedido.',decoded,code};if(!Number.isFinite(amount)||amount!==Number(amountCents))return{ok:false,error:'Redsys devolvió un importe distinto.',decoded,code};if(currency!=='978')return{ok:false,error:'Redsys devolvió otra moneda.',decoded,code};if(tx!=='3')return{ok:false,error:'Redsys devolvió otro tipo de operación.',decoded,code};if(responseMerchant&&responseMerchant!==String(merchant))return{ok:false,error:'Redsys devolvió otro comercio.',decoded,code};if(responseTerminal&&responseTerminal!==String(terminal))return{ok:false,error:'Redsys devolvió otro terminal.',decoded,code};if(code!=='0900')return{ok:false,error:'Redsys no confirmó la devolución.',decoded,code};return{ok:true,decoded,code};
}
...
exports.handler=async event=>{
 if(event.httpMethod==='OPTIONS')return{statusCode:204,headers:CORS,body:''};if(event.httpMethod!=='POST')return resp(405,{error:'Method Not Allowed'});
 const auth=await requireStaff(event,'returns.manage');if(!auth.ok)return resp(auth.statusCode,{error:auth.error});const elevated=await verifyStepUp(event,auth);if(!elevated.ok)return resp(elevated.statusCode,{error:elevated.error,code:elevated.code});
 ...
 const payment=await paymentConfig.resolve().catch(()=>null),secret=payment?.secretKey,merchant=payment?.merchantCode,terminal=payment?.terminal||'1',env=payment?.environment||'test';if(!payment?.enabled||!secret||!merchant)return resp(503,{error:'Redsys no está configurado.'});if(payment.managed&&env==='production'&&payment.dedicatedVaultKey!==true)return resp(503,{error:'La bóveda de pagos de producción no tiene una clave dedicada configurada.'});
 ...
 try{return await withLock(`refund:${orderId}`,async()=>{
  ...
  const integrity=await defense.verifyAuditIntegrity();if(!integrity.ok)return resp(integrity.statusCode,{error:integrity.error,code:integrity.code});const replay=await defense.consumeMutationNonce({event,actor:auth.email,scope:`refund:${orderId}:${amountCents}`});if(!replay.ok)return resp(replay.statusCode,{error:replay.error,code:replay.code});
  ...
  const checked=verifyRefundResponse(data,{secret,orderId,amountCents,merchant,terminal});if(!r.ok||!checked.ok){await incident(orderId,amountCents,{responseCode:checked.code||null,error:checked.error||`HTTP ${r.status}`});return resp(502,{error:checked.error||'Redsys no confirmó la devolución.',responseCode:checked.code||null})}
  const decoded=checked.decoded,code=checked.code,nextRefunded=refunded+amountCents,refund={id:crypto.randomUUID(),at:new Date().toISOString(),amountCents,by:auth.email,responseCode:code,authorisationCode:field(decoded,'Ds_AuthorisationCode','DS_AUTHORISATIONCODE')||null},updated={...order,refundedCents:nextRefunded,refunds:[...(order.refunds||[]),refund],refundStatus:nextRefunded===paid?'REFUNDED':'PARTIALLY_REFUNDED'};
  ...
  const invoiceId=`refund:${orderId}:${refund.id}`;await enterprise.save('invoices',{id:invoiceId,orderId,customerEmail:order.email||'',currency:'EUR',status:'issued',type:'credit_note',amountCents:-amountCents,issuedAt:new Date().toISOString(),reference:refund.id},auth,{id:invoiceId,reason:'redsys-refund'}).catch(()=>{});return resp(200,{ok:true,orderId,refundedCents:nextRefunded,remainingCents:paid-nextRefunded,refund});
