...
<html lang="es"><head>
  ...
  <link rel="stylesheet" href="/styles.css"><link rel="stylesheet" href="/commercial-finish.css">
  ${schemas}
...
</head><body><header class="nt-seo-head"><div class="nt-seo-wrap"><a class="nt-seo-brand" href="/">NUTRE<span>TIUM</span></a><a class="nt-seo-nav" href="/#products">Tienda</a></div></header>${body}<footer class="nt-seo-foot"><div class="nt-seo-wrap">Nutretium · BAHÍA NORTE CAPITAL, S.L. · C/ La Albericia 1, Santander · <a class="nt-seo-nav" href="/ayuda">Ayuda</a></div></footer></body></html>`;
}
...
function collectionPage({kind, label, slug, list, description}) {
  ...
  const body = `<main class="nt-seo-wrap"><section class="nt-seo-hero"><span class="nt-seo-kicker">Nutretium · catálogo online</span><h1>${esc(label)}</h1><p>${esc(description)}</p></section><section class="nt-seo-grid">${cards(list)}</section>${links}</main>`;
...
function buildSitemap(brands) {
  const urls = new Set([`${ORIGIN}/`,`${ORIGIN}/ayuda`,`${ORIGIN}/aprende`,`${ORIGIN}/recomendador`]);
