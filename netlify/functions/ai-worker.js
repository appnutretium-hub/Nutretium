...
exports.handler=async function(){
 try{
  ...
  try{generative=await ollama.enqueuePortfolio({runs:runtime.PORTFOLIO_RUNS})}
  catch(error){generative={enabled:true,error:String(error.message||error).slice(0,300),queued:0,reused:0,jobs:[]}}
  ...
  return{statusCode:failed===portfolio.results.length?500:200,headers:{'content-type':'application/json','cache-control':'no-store'},body:JSON.stringify({ok:failed===0,partial:failed>0&&failed<portfolio.results.length,agents:portfolio.results.length,failed,dataQuality:portfolio.dataQuality,externalSpendLimitEur:0,generative,growthLoop,federationCouncil,results:portfolio.results})};
 }catch(error){
