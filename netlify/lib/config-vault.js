...
async function paymentStatus(){const value=await readPayment();return{vaultConfigured:configured(),dedicatedKeyConfigured:Boolean(master()),merchantCodeConfigured:Boolean(value?.merchantCode),secretKeyConfigured:Boolean(value?.secretKey),updatedAt:value?.updatedAt||null}}
