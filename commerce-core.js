...
(function () {
  ...
  function handleProductPageQuantity(){const params=new URLSearchParams(location.search);const id=Number(params.get('add'));const requested=Math.max(1,Math.min(20,Number(params.get('qty'))||1));if(!Number.isInteger(id)||requested<=1)return;setTimeout(()=>{if(typeof window.addToCart!=='function')return;for(let i=1;i<requested;i+=1)window.addToCart(id);},150);}
