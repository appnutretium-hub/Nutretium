...
(async()=>{
  ...
  assert.strictEqual(genesis.version,1,'Debe crear checkpoint inicial.');
  ...
  assert.strictEqual(promoted.version,2,'La promoción debe crear nueva versión.');
  ...
  assert.strictEqual(freshPromotion.version,3);
  ...
  assert.strictEqual(rollback.version,4,'Rollback crea nueva versión, no destruye historial.');
