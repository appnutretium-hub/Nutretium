...
async function applyRuntime(){const p=await resolve();if(!p.managed)return p;process.env.REDSYS_ENV=p.environment;process.env.COMMERCE_LIVE=p.enabled&&p.commerceLive?'true':'false';process.env.REDSYS_MERCHANT_CODE=p.merchantCode||'';process.env.REDSYS_SECRET_KEY=p.secretKey||'';process.env.REDSYS_TERMINAL=p.terminal||'1';for(const[name,value]of[['MERCHANT_URL',p.merchantUrl],['URL_OK',p.urlOk],['URL_KO',p.urlKo]]){if(value)process.env[name]=value;else delete process.env[name]}return p}
