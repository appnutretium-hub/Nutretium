...
(async()=>{
 ...
 assert.equal(readiness.factusolState({}).ready,false);
 assert.equal(readiness.factusolState({ready:true,configured:true,connected:true,exercise:'2026'}).ready,true);
 assert.equal(readiness.factusolState({ready:true,configured:true,connected:false,exercise:'2026'}).ready,false);
 assert.equal(readiness.catalogState({ready:true,active:80}).ready,true);
 assert.equal(readiness.complianceState({ready:true,strict:true,blocked:0}).ready,true);
 assert.equal(readiness.complianceState({ready:true,strict:false,blocked:0}).ready,false);
 ...
 assert.equal((await readiness.githubDeploymentState({GITHUB_REPOSITORY:'appnutretium-hub/Nutretium',GITHUB_TOKEN:'x',COMMIT_REF:sha,CONTEXT:'production',BRANCH:'main'},gh)).ready,true);
 const mismatch=await readiness.githubDeploymentState({GITHUB_REPOSITORY:'appnutretium-hub/Nutretium',GITHUB_TOKEN:'x',COMMIT_REF:'b'.repeat(40),CONTEXT:'production',BRANCH:'main'},gh);
 assert.equal(mismatch.reason,'sha-mismatch');
 const verified=await readiness.resendState({RESEND_API_KEY:'x',ORDER_EMAIL_FROM:'Nutretium <pedidos@nutretium.com>'},router([['api.resend.com',{data:[{name:'nutretium.com',status:'verified'}]}]]));
 ...
 const aggregate=await readiness.assessExternalReadiness({env:{GITHUB_REPOSITORY:'appnutretium-hub/Nutretium',GITHUB_TOKEN:'x',COMMIT_REF:sha,CONTEXT:'production',BRANCH:'main',RESEND_API_KEY:'x',ORDER_EMAIL_FROM:'pedidos@nutretium.com',REDSYS_ENV:'production',COMMERCE_LIVE:'true',REDSYS_SECRET_KEY:'secret',REDSYS_MERCHANT_CODE:'merchant'},shipping:{managed:true,enabled:true,rateCents:495},factusol:{ready:true,configured:true,connected:true,exercise:'2026'},catalog:{ready:true,active:80,presentationReady:false},compliance:{ready:true,strict:true,blocked:0},fetchImpl:aggregateFetch});
 ...
 assert.ok(failClosed.blockers.length>=7);
