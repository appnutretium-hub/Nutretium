...
// Fichas, landings, sitemap y robots los genera el build en la raíz. Para no
// depender de que se haya ejecutado (ni probar lo que dejó un build antiguo),
// se generan aquí mismo, con el código actual, en una carpeta temporal.
const salida = fs.mkdtempSync(path.join(os.tmpdir(), 'nutretium-seo-'));
fs.copyFileSync(path.join(root, 'producto.html'), path.join(salida, 'producto.html'));
execFileSync(process.execPath, [path.join(__dirname, 'build-seo-pages.js')], { cwd: salida, stdio: 'ignore' });
...
const productPage = path.join(salida,'producto',`${pim.slugify(first.name)}-${first.id}`,'index.html');
...
assert(productHtml.includes('/product-content-ui.js'), 'La ficha estática debe cargar product-content-ui.js (antes solo lo recibía la plantilla, después de generar las fichas)');
...
const categoryPage = path.join(salida,'categoria',pim.slugify(category),'index.html');
...
if (brands.length) assert(fs.existsSync(path.join(salida,'marca',pim.slugify(brands[0]),'index.html')), 'Debe generarse landing de marca');
...
const robots = fs.readFileSync(path.join(salida,'robots.txt'),'utf8');
