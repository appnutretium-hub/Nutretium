...
exports.handler=async function(){
 try{
  ...
  const snapshot=await enterprise.snapshot();snapshot.checksum=checksum(snapshot);
  ...
  const records=Object.values(snapshot.counts||{}).reduce((sum,n)=>sum+Number(n||0),0);
  return{statusCode:200,body:JSON.stringify({ok:true,key,domains:Object.keys(snapshot.domains).length,records,checksum:snapshot.checksum})};
