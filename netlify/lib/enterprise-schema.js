...
const DOMAIN_DEFS = Object.freeze({
  ...
  'product-variants':{permission:'inventory.manage',required:['productId','sku'],numeric:['priceCents','costCents','weightGrams'],statuses:['active','inactive','discontinued']},
  'price-history':{permission:'finance.read',required:['sku','priceCents','effectiveAt'],numeric:['priceCents','previousPriceCents']},
  'commerce-settings':{permission:'platform.write',required:['key'],statuses:['active','inactive']},
...
const MAX_TEXT=5000,MAX_ARRAY=500,MAX_DEPTH=8,RESERVED=new Set(['__proto__','prototype','constructor']);
function cleanValue(value,depth=0){if(depth>MAX_DEPTH)throw new Error('La estructura supera la profundidad permitida.');if(value===null||typeof value==='boolean'||typeof value==='number')return value;if(typeof value==='string')return value.slice(0,MAX_TEXT).trim();if(Array.isArray(value)){if(value.length>MAX_ARRAY)throw new Error('La lista supera el máximo permitido.');return value.map(v=>cleanValue(v,depth+1));}if(typeof value==='object'){const out={};for(const[key,val]of Object.entries(value)){if(RESERVED.has(key))continue;if(key.length>80)throw new Error('Hay un nombre de campo demasiado largo.');out[key]=cleanValue(val,depth+1);}return out;}return String(value).slice(0,MAX_TEXT);}
function validate(domain,raw,{partial=false}={}){const def=DOMAIN_DEFS[domain];if(!def)return{ok:false,errors:['Dominio no reconocido.']};let data;try{data=cleanValue(raw||{});}catch(err){return{ok:false,errors:[err.message]};}const errors=[];if(!partial)for(const field of def.required||[]){const v=data[field];if(v===undefined||v===null||v===''||(Array.isArray(v)&&!v.length))errors.push(`Falta ${field}.`);}if(data.status!==undefined&&def.statuses&&!def.statuses.includes(data.status))errors.push(`Estado no válido para ${domain}.`);for(const field of def.numeric||[])if(data[field]!==undefined&&data[field]!==null&&(!Number.isFinite(Number(data[field]))||Number(data[field])<0))errors.push(`${field} debe ser un número no negativo.`);
if(domain==='promotions'){const types=['percent','fixed','free_shipping','bundle','gift'];if(data.type&&!types.includes(data.type))errors.push('Tipo de promoción no válido.');if(data.type==='percent'&&(Number(data.value)<=0||Number(data.value)>100))errors.push('El porcentaje debe estar entre 0 y 100.');if(data.type==='fixed'&&Number(data.value)<=0)errors.push('El descuento fijo debe ser mayor que cero.');}
if(domain==='product-compliance'&&data.status==='approved'&&(!data.evidenceComplete||!Array.isArray(data.evidence)||!data.evidence.length))errors.push('No se puede aprobar cumplimiento sin evidencia documental completa.');
...
if(domain==='product-variants'&&data.ean&& !/^\d{8,14}$/.test(String(data.ean)))errors.push('EAN/GTIN debe tener entre 8 y 14 dígitos.');
if(domain==='commerce-settings'){for(const k of ['loyaltyCentPerPoint','loyaltyEarnPointsPerEuro','referralBuyerDiscountCents','referralOwnerRewardPoints'])if(data[k]!==undefined&&(!Number.isInteger(Number(data[k]))||Number(data[k])<0))errors.push(`${k} debe ser un entero no negativo.`);}
if(domain==='webhooks'&&data.url){try{if(new URL(data.url).protocol!=='https:')errors.push('Los webhooks deben usar HTTPS.');}catch{errors.push('La URL del webhook no es válida.');}}
if(domain==='seo-redirects'&&data.from&&data.to&&data.from===data.to)errors.push('Una redirección no puede apuntarse a sí misma.');return{ok:errors.length===0,errors,data};}
function definition(domain){return DOMAIN_DEFS[domain]||null}function domains(){return Object.keys(DOMAIN_DEFS)}module.exports={DOMAIN_DEFS,validate,definition,domains,cleanValue};
