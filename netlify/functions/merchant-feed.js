...
function eligibleBase(p){return Boolean(p&&p.active!==false&&Number(p.price)>0&&String(p.name||'').trim()&&String(p.image||'').trim()&&typeof p.stock==='number'&&Number.isFinite(p.stock));}
function renderItem(p,content={}){
  if(!eligibleBase(p))return null;
  ...
  const approved=content&&content.status==='approved';
  const desc=String((approved&&content.description)||p.description||`${p.name}${p.category?` — ${p.category}`:''}`).slice(0,5000);
  const gtin=approved&&/^\d{8,14}$/.test(String(content.ean||''))?String(content.ean):'';
  const category=p.category?`<g:product_type>${x(p.category)}</g:product_type>`:'';
  return `<item><g:id>${x(p.code||p.id)}</g:id><g:title>${x(p.name)}</g:title><g:description>${x(desc)}</g:description><g:link>${x(link)}</g:link><g:image_link>${x(image)}</g:image_link><g:condition>new</g:condition><g:availability>${availability}</g:availability><g:price>${Number(p.price).toFixed(2)} EUR</g:price>${p.brand?`<g:brand>${x(p.brand)}</g:brand>`:''}${gtin?`<g:gtin>${x(gtin)}</g:gtin>`:''}${category}</item>`;
}
exports.handler=async function(){
  const base='https://nutretium.com';
  const statuses=await compliance.statuses();
  const candidates=(NUTRETIUM_PRODUCTS||[]).filter((p)=>{
    if(!eligibleBase(p))return false;
    const state=statuses.get(String(p.code||'').trim().toUpperCase());
    return !state||!['blocked','rejected'].includes(state.status);
  });
  const rendered=await Promise.all(candidates.map(async p=>{
    const content=await productContent.read(p.code).catch(()=>null);
    return renderItem(p,content||{});
  }));
  const items=rendered.filter(Boolean).join('');
...
exports._test={eligibleBase,renderItem};
