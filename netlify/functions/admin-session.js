...
exports.handler=async function(event){
 ...
 if(body.action==='exchange'){
  ...
  const recoveryAllowed=temporaryAccess.claimAllowsPrivilegedBypass(role,verified.claims);
  if(mfaRequired&&verified.claims.mfa!==true&&!recoveryAllowed)return json(401,{error:'La sesión interna requiere autenticación MFA verificada.',code:'STAFF_MFA_REQUIRED'});
  const now=Math.floor(Date.now()/1000),csrf=security.randomToken(32),binding=defense.newSessionBinding(event),token=signJWT({sub:verified.user.id,email:verified.email,role,kind:'staff',mfa:verified.claims.mfa===true,mfaBypass:recoveryAllowed,mfaBypassUntil:recoveryAllowed?temporaryAccess.MFA_BYPASS_UNTIL:0,csrf,sid:binding.sid,fp:binding.fp,jti:binding.jti,sv:Number(verified.user.sessionVersion||0),exp:now+MAX_AGE});
  return json(200,{ok:true,email:verified.email,role,permisos:[...current.permissions],expiresIn:MAX_AGE,csrf,mfaRecovery:recoveryAllowed,mfaBypassUntil:recoveryAllowed?temporaryAccess.MFA_BYPASS_UNTIL_ISO:null},{'Set-Cookie':cookie(token,MAX_AGE)});
 }
 if(body.action==='status'){
  ...
  return json(200,{ok:true,email:verified.email,role:current.member.role,permisos:[...current.permissions],csrf:verified.claims.csrf||null,expiresAt:verified.claims.exp||null,mfa:verified.claims.mfa===true,mfaRecovery:verified.claims.mfaBypass===true});
 }
