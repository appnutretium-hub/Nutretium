...
exports.handler=async function(event){
  ...
  const store=getBlobStore('saved-carts');
  if(!store) return json(503,{error:'Carrito guardado no disponible.'});

  if(event.httpMethod==='GET'){
    const cart=await store.get(email,{type:'json'}).catch(()=>null);
    return json(200,{cart:cart||null});
  }
  if(event.httpMethod==='POST'){
    ...
    const priced=valorarCarrito(items);
    if(!priced.ok) return json(400,{error:priced.errores[0],detalles:priced.errores});
    const record={email,items:priced.lineas,total:priced.totalCents/100,updatedAt:new Date().toISOString()};
    await store.setJSON(email,record);
    return json(200,{ok:true,cart:record});
  }
