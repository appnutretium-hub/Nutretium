...
async function staffRows(){
 ...
 const out=[];for(const row of rows){const user=await usuarios.lee(row.email).catch(()=>null);out.push({...row,accountExists:Boolean(user),emailVerified:Boolean(user?.emailVerifiedAt),passwordChangedAt:user?.passwordChangedAt||null})}return out;
}
...
exports.handler=async event=>{
 ...
 try{
  ...
  if(action==='staff-save'){
   ...
   const saved=await directory.save(body.record||{});await audit.append({event,actor:auth.email,action:'STAFF_ACCESS_UPDATED',resource:saved.email,outcome:'SUCCESS',metadata:{role:saved.role,active:saved.active,permissions:saved.permissions}});return response(200,{record:saved});
  }
 ...
 }catch(err){console.error('[admin-governance]',err);return response(500,{error:err.message||'No se pudo completar la operación.'})}
};
