...
function mfaRequiredFor(role,user){return !privilegedMfaExempt(role)&&(['owner','admin'].includes(String(role||''))||security.staffMfaRequired()||user?.mfaEnabled===true)}
...
exports.handler = async event => {
  ...
  if (!wait.ok) return response(wait.degraded ? 429 : 429,{error:'Demasiados intentos. Prueba más tarde.'},{ 'Retry-After': String(wait.retryAfter) });
