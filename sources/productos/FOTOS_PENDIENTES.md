# Fotos que faltan

> Generado por `scripts/fotos-incorporar.js`. No editar a mano: se rehace
> solo cada vez que se incorporan fotos nuevas.

**83 de 152 productos siguen sin foto** (69 ya la tienen).

Se parten en dos grupos que se resuelven de forma distinta. Para meter las
fotos una vez conseguidas, ver `sources/_nuevas/LEEME.md`.

---

## A. Pedir el packshot al distribuidor — 32 productos

Producto de marca envasado: el fabricante tiene foto oficial y material de
punto de venta, y con él viene el permiso de uso. Es de donde salieron las
30 fotos AMRO que ya están publicadas. Cada apartado de abajo es la lista
que hay que pedirle a ese proveedor.

### Protzen — 8

| Código | Producto | Categoría |
|---|---|---|
| `00496` | Helado Chocolate Negro 100ml Protzen | Helados |
| `00490` | Helado Chocolate Negro 460ml Protzen | Helados |
| `00491` | Helado Moka Fuel 100ml Protzen | Helados |
| `00485` | Helado Moka Fuel 460ml Protzen | Helados |
| `00486` | Helado Pistacho 460ml Protzen | Helados |
| `00492` | Helado Pistacho100ml Protzen | Helados |
| `00495` | Helado Vainilla 100ml Protzen | Helados |
| `00489` | Helado Vainilla 460ml Protzen | Helados |

### Protella — 6

| Código | Producto | Categoría |
|---|---|---|
| `00302` | Crema De Proteina Bonbon Crisp Protella 200g | Alimentación proteica |
| `00300` | Crema De Proteina Chocolate Blanco Protella 200g | Alimentación proteica |
| `00303` | Crema De Proteina Cookie Dough Protella 200g | Alimentación proteica |
| `00331` | Colageno Mango 180g Protella | Colágeno y bienestar |
| `00298` | Mermelada Melocoton Protella 170g | Alimentación proteica |
| `00299` | Mermelada Strawberry Protella 170g | Alimentación proteica |

### Barebells — 5

| Código | Producto | Categoría |
|---|---|---|
| `00350` | Milkshake Cookies & Cream 330ml Barebells | Bebidas |
| `00346` | Milkshake Strawberry 330ml Barebells | Bebidas |
| `00349` | Milkshake Vainilla 330ml Barebells | Bebidas |
| `00304` | Barrita Proteina Lemon Cheesecake 16g Barebells | Barritas y snacks |
| `00310` | Barrita Proteina Strawberry Sundae 16g Barebells | Barritas y snacks |

### Vitamin Well — 5

| Código | Producto | Categoría |
|---|---|---|
| `00358` | Vitamin Well Boost Arándanos Y Frambuesa | Bebidas |
| `00357` | Vitamin Well Refresh Limonada/kiwi | Bebidas |
| `00360` | Vitamin Well Sport 002 Limón/lima | Bebidas |
| `00361` | Vitamin Well Zero Limón | Bebidas |
| `00359` | Vitamin Well Zero Peach Melocotón | Bebidas |

### Aquadeus — 4

| Código | Producto | Categoría |
|---|---|---|
| `00532` | Agua Aquadeus 1.5l | Bebidas |
| `00500` | Agua Aquadeus 1.5l | Bebidas |
| `00480` | Oferta Pack Aqua Deus 6 Botellas 1.5l | Bebidas |
| `00539` | Pack Aquadeus 6 Botellas 1.5l | Bebidas |

### Solares — 2

| Código | Producto | Categoría |
|---|---|---|
| `00531` | Agua Solares Pequeña 0.50ml | Bebidas |
| `00533` | Agua Solares 1.5l | Bebidas |

### NABE — 1

| Código | Producto | Categoría |
|---|---|---|
| `00363` | NABE Aloe Vera THE Original | Bebidas |

### Sin marca identificada — 1

| Código | Producto | Categoría |
|---|---|---|
| `00155` | Pastillero | Accesorios gym |

---

## B. Fotografiar en la tienda — 51 productos

Recetas y elaboraciones de la casa: **no existen en internet**. Nadie tiene
una foto de un «Royal X 750 ml» salvo Nutretium. Con el móvil, fondo liso y
luz de ventana es suficiente; los consejos están en `sources/_nuevas/LEEME.md`.

Un mismo plato en varios tamaños puede compartir foto: se nombra el archivo
con los códigos separados por `+`.

| Código | Producto | Categoría |
|---|---|---|
| `ABB-1000` | Açaí Berry Brutal 1000 ml | Bowls |
| `ABB-750` | Açaí Berry Brutal 750 ml | Bowls |
| `BG-1000` | Bahía Gold 1000 ml | Bowls |
| `BG-500` | Bahía Gold 500 ml | Bowls |
| `BG-750` | Bahía Gold 750 ml | Bowls |
| `BCB-1000` | Black Cacao Berry 1000 ml | Bowls |
| `BCB-500` | Black Cacao Berry 500 ml | Bowls |
| `BCB-750` | Black Cacao Berry 750 ml | Bowls |
| `CP-1000` | Choco Peanut 1000 ml | Bowls |
| `CP-500` | Choco Peanut 500 ml | Bowls |
| `CP-750` | Choco Peanut 750 ml | Bowls |
| `DP-1000` | Dubai Pistachio 1000 ml | Bowls |
| `DP-500` | Dubai Pistachio 500 ml | Bowls |
| `DP-750` | Dubai Pistachio 750 ml | Bowls |
| `MS-1000` | Matcha Strawberry 1000 ml | Bowls |
| `MS-500` | Matcha Strawberry 500 ml | Bowls |
| `MS-750` | Matcha Strawberry 750 ml | Bowls |
| `RX-1000` | Royal X 1.000 ml | Bowls |
| `RX-500` | Royal X 500 ml | Bowls |
| `RX-750` | Royal X 750 ml | Bowls |
| `BOWL-1000` | Personaliza Bowl 1.000 ml | Bowls |
| `BOWL-500` | Personaliza Bowl 500 ml | Bowls |
| `BOWL-750` | Personaliza Bowl 750 ml | Bowls |
| `DAILY-BERRY` | Daily Berry 500 ml | Yogures |
| `DAILY-CACAO` | Daily Cacao Crunch 500 ml | Yogures |
| `DAILY-TROPI` | Daily Tropical 500 ml | Yogures |
| `00538` | Batido Chocolate | Smoothies y batidos |
| `00537` | Batido Mango | Smoothies y batidos |
| `BATI-BC` | Berry Cloud | Smoothies y batidos |
| `BATI-CP` | Cacao Peanut | Smoothies y batidos |
| `CAPRI-BCP` | Berry Cloud Pop | Waffles y caprichos |
| `00536` | Capricho Fresa | Waffles y caprichos |
| `CAPRI-MSC` | Mini Sphere Cup | Waffles y caprichos |
| `CAPRI-PGP` | Pistachio Gold Pop | Waffles y caprichos |
| `CAFE-AMERICAN` | Americano | Café y matcha |
| `CAFE-CAPPUCCI` | Cappuccino | Café y matcha |
| `CAFE-CORTADO` | Cortado | Café y matcha |
| `CAFE-ESPRESO` | Espresso | Café y matcha |
| `CAFE-ICED` | LAT Iced Latte | Café y matcha |
| `CAFE-LATTE` | Latte | Café y matcha |
| `00001` | Agua Pequeña 33ml | Bebidas |
| `ICE-CC` | Iced Cacao Coffee | Bebidas |
| `ICE-MC` | Iced Matcha Classic | Bebidas |
| `ICE-PM` | Iced Pistachio Matcha | Bebidas |
| `ICE-MT` | Mango Tropical | Bebidas |
| `00083` | Zumo Naranja 500ml | Bebidas |
| `00284` | Cafe + Leche | Café y matcha |
| `00095` | Dip Extra Salsa | Waffles y caprichos |
| `00086` | Bolw Grande | Bowls |
| `00085` | Bolw Mediano | Bowls |
| `00084` | Bolw Pequeño | Bowls |
