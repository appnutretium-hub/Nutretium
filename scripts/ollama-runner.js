...
async function processJob(job){
 let finished=false;const heartbeat=setInterval(()=>{if(!finished)bridge({action:'heartbeat',id:job.id,leaseId:job.leaseId,leaseMs:LEASE_MS}).catch(()=>{})},45000);
 ...
 finally{finished=true;clearInterval(heartbeat)}
}
...
async function main(){assertConfig();await health();console.log(`[ollama-runner] ${RUNNER_ID} conectado a Nutretium`);for(;;){try{const {job}=await bridge({action:'claim',leaseMs:LEASE_MS});if(job)await processJob(job);else await sleep(7000)}catch(error){console.error(`[ollama-runner] ${String(error?.message||error).slice(0,300)}`);await sleep(10000)}}}
