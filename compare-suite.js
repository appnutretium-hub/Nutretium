...
function decorate(){
  ...
  document.querySelectorAll('#productGrid .product-card').forEach(card=>{
    ...
    if(!btn){btn=document.createElement('button');btn.type='button';btn.className='nt-compare-btn';btn.dataset.productId=id;btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();toggle(id)});const action=card.querySelector('.p-5')||card;action.appendChild(btn)}
    const on=selected.has(id);btn.classList.toggle('active',on);btn.setAttribute('aria-pressed',String(on));btn.textContent=on?'✓ En comparación':'+ Comparar';
  });
}
function tray(){
  ...
  el.innerHTML=`<div><strong>${ids.length}/${MAX} para comparar</strong><span>${products.map(p=>String(p.name).replace(/[<>]/g,'')).join(' · ')}</span></div><div class="nt-compare-actions"><button type="button" id="ntClearCompare">Limpiar</button><a href="/comparar">Comparar ahora</a></div>`;
  el.querySelector('#ntClearCompare').onclick=()=>{write([]);decorate();tray()};
}
function observe(){const grid=document.getElementById('productGrid');if(!grid)return;new MutationObserver(()=>decorate()).observe(grid,{childList:true,subtree:true})}
