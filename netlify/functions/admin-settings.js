...
async function stepUpOrResponse(event,staff){const gate=await verifyStepUp(event,staff);return gate.ok?null:json(gate.statusCode,{error:gate.error,code:gate.code})}
exports.handler=async function(event){
 ...
 if(event.httpMethod==='POST'){
  ...
  if(body.action==='save-payment-secret'||body.action==='clear-payment-secret'){
   ...
   const step=await stepUpOrResponse(event,staff);if(step)return step;
  ...
  const current=await settings.readWithVersion().catch(()=>({settings:null}));
  const paymentBecomesSensitive=next.payment.managed&&next.payment.enabled&&(next.payment.environment==='production'||next.payment.commerceLive)&&(!current.settings||current.settings.payment.environment!==next.payment.environment||current.settings.payment.commerceLive!==next.payment.commerceLive||current.settings.payment.enabled!==next.payment.enabled);
  if(paymentBecomesSensitive){if(staff.role!=='owner')return json(403,{error:'Solo el propietario puede activar o modificar pagos de producción.'});const step=await stepUpOrResponse(event,staff);if(step)return step}
