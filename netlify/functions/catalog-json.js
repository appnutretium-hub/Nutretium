...
function availability(p){return typeof p.stock==='number'&&Number.isFinite(p.stock)?(p.stock>0?'https://schema.org/InStock':'https://schema.org/OutOfStock'):'https://schema.org/LimitedAvailability'}
function item(p){return {'@type':'Product','@id':`https://nutretium.com/producto/${slug(p.name)}-${p.id}`,name:String(p.name),sku:String(p.code||p.id),category:String(p.category||''),brand:p.brand?{'@type':'Brand',name:String(p.brand)}:undefined,image:p.image?`https://nutretium.com/${String(p.image).replace(/^\//,'')}`:undefined,offers:{'@type':'Offer',url:`https://nutretium.com/producto/${slug(p.name)}-${p.id}`,priceCurrency:'EUR',price:Number(p.price).toFixed(2),availability:availability(p)}}}
