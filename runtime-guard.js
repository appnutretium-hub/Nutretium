...
function keyboardRoleButtons(){document.addEventListener('keydown',e=>{const el=e.target?.closest?.('[role="button"][tabindex="0"]');if(!el)return;if(e.key==='Enter'||e.key===' '){e.preventDefault();el.click()}})}
function init(){status();externalLinks();keyboardRoleButtons();requestAnimationFrame(touchTargets)}
