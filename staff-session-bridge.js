...
(function(){
  ...
  async function exchangeStaffToken(token){
    const exchange=await nativeFetch(SESSION_ENDPOINT,{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({action:'exchange'})});
  ...
  async function ensureCsrf(){
    ...
    csrfPromise=(async()=>{const r=await nativeFetch(SESSION_ENDPOINT,{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'status'})});if(!r.ok)return null;const data=await r.json().catch(()=>({}));csrfToken=data.csrf||null;return csrfToken})().finally(()=>{csrfPromise=null});
    return csrfPromise;
  }
  async function performStepUp(){
    const csrf=await ensureCsrf().catch(()=>null);if(!csrf)return false;
    const code=window.prompt('Operación crítica de Nutretium. Introduce el código MFA de 6 dígitos:');if(!code)return false;
  ...
  async function sendWithSecurity(input,init,url,method,allowStepUp=true){
    let nextInit=init;if(needsCsrf(url,method)){const csrf=await ensureCsrf().catch(()=>null);if(csrf){const headers=new Headers(init?.headers||input?.headers||{});headers.set('X-Nutretium-CSRF',csrf);nextInit={...init,headers,credentials:'same-origin'}}}
