...
function callbackIntegrity(params, record, payment){
  const expected={
    currency:clean(record?.currency||'978'),
    merchantCode:clean(payment?.merchantCode),
    terminal:clean(payment?.terminal||'1')
  };
  const received={
    currency:clean(params?.Ds_Currency??params?.DS_CURRENCY),
    merchantCode:clean(params?.Ds_MerchantCode??params?.DS_MERCHANTCODE),
    terminal:clean(params?.Ds_Terminal??params?.DS_TERMINAL)
  };
  const issues=[];
  if(!received.currency||received.currency!==expected.currency)issues.push({code:'CURRENCY_MISMATCH',expected:expected.currency,received:received.currency||null});
  if(expected.merchantCode&&received.merchantCode!==expected.merchantCode)issues.push({code:'MERCHANT_MISMATCH',expected:expected.merchantCode,received:received.merchantCode||null});
  if(expected.terminal&&received.terminal!==expected.terminal)issues.push({code:'TERMINAL_MISMATCH',expected:expected.terminal,received:received.terminal||null});
  return{ok:issues.length===0,issues};
}
async function sendOrderEmails(record){const results={store:false,customer:false};if(record.missingOrderRecord)return results;try{const storeMail=buildStoreOrderEmail(record),r=await sendEmail({to:process.env.ORDER_NOTIFICATION_EMAIL,subject:storeMail.subject,html:storeMail.html,idempotencyKey:`nutretium-order-store/${record.order}`});results.store=Boolean(r?.ok)}catch(err){console.error('[Redsys-notify] Email tienda:',err)}if(record.email&&!record.amountMismatch&&!record.paymentIntegrityMismatch&&!record.missingOrderRecord){try{const customerMail=buildCustomerOrderEmail(record),r=await sendEmail({to:record.email,subject:customerMail.subject,html:customerMail.html,idempotencyKey:`nutretium-order-customer/${record.order}`});results.customer=Boolean(r?.ok)}catch(err){console.error('[Redsys-notify] Email cliente:',err)}}return results}
exports.handler=async function(event){
 ...
 const transactionType=String(params.Ds_TransactionType??params.DS_TRANSACTIONTYPE??'');if(transactionType&&transactionType!=='0'){console.warn('[Redsys-notify] Tipo de operación ajeno al cobro inicial; se ignora sin mutar el pedido',order,transactionType);return{statusCode:200,headers:HEADERS,body:'OK'}}
 const responseCode=parseInt(params.Ds_Response,10),authorised=Number.isInteger(responseCode)&&responseCode>=0&&responseCode<=99,bankAmount=Number(params.Ds_Amount)/100,bankCurrency=params.Ds_Currency??params.DS_CURRENCY;
...
exports._test={isEnterpriseOrder,callbackIntegrity};
