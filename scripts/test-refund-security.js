...
function signedNotification(order,amount,response='0000'){
 const params=Buffer.from(JSON.stringify({Ds_Amount:String(amount),Ds_Currency:'978',Ds_Order:order,Ds_Response:response,Ds_AuthorisationCode:'ABC123'})).toString('base64');
 return{Ds_SignatureVersion:'HMAC_SHA256_V1',Ds_MerchantParameters:params,Ds_Signature:refund.sign(params,secret,order)};
}
(async()=>{
 ...
 const orders=getBlobStore('redsys-orders'),mismatchOrder='0920MISMATCH';
 await orders.setJSON(mismatchOrder,{order:mismatchOrder,email:'buyer@example.com',items:[{id:1,code:'SKU',name:'Producto',qty:1}],amount:2.95,currency:'978',status:'PENDING',inventoryReservation:{status:'RESERVED'}});
 const bank=signedNotification(mismatchOrder,294);
 const result=await notify.handler({httpMethod:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(bank)});
 assert.strictEqual(result.statusCode,200,'Una notificación firmada discrepante debe quedar registrada para revisión');
 const held=await orders.get(mismatchOrder,{type:'json'});
 assert.strictEqual(held.fulfilmentStatus,'REVIEW_REQUIRED');
 assert.strictEqual(held.inventoryReservation.status,'HELD_REVIEW','La reserva no debe consumirse ni liberarse mientras se concilia el cobro');
 assert.strictEqual(held.paymentReview.held,true);
 console.log('[test-refund-security] OK · firma/importe/pedido Redsys · lock de reembolso · reserva retenida ante discrepancia');
