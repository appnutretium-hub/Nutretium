...
const staffForgedBypass=signJWT({sub:user.id,email,kind:'staff-login',mfa:false,mfaBypass:true,mfaBypassUntil:Date.now()+86400000,exp:Math.floor(Date.now()/1000)+3600});
...
(async()=>{
 ...
 assert.strictEqual(staffLogin._test.privilegedMfaExempt('owner'),false,'Owner nunca debe quedar exento de MFA');
 assert.strictEqual(staffLogin._test.privilegedMfaExempt('admin'),false,'Admin nunca debe quedar exento de MFA');
 assert.strictEqual(staffLogin._test.privilegedMfaExempt('manager'),false,'No existe bypass MFA por rol');
 ...
 assert.strictEqual(noMfaExchange.statusCode,401,'Un token de personal sin MFA debe rechazarse');
 const forgedBypassExchange=await adminSession.handler(event({action:'exchange'},{authorization:`Bearer ${staffForgedBypass}`}));
 assert.strictEqual(forgedBypassExchange.statusCode,401,'Claims heredados o manipulados de bypass MFA deben ignorarse y rechazarse');

 const exchange=await adminSession.handler(event({action:'exchange'},{authorization:`Bearer ${staffBearer}`}));
 assert.strictEqual(exchange.statusCode,200,'El intercambio debe aceptar staff-login verificado con MFA');
 assert.strictEqual(parse(exchange).mfa,true,'La sesión interna debe conservar evidencia MFA');
 assert.strictEqual(Object.prototype.hasOwnProperty.call(parse(exchange),'mfaRecovery'),false,'La API no debe exponer estados de recuperación/bypass MFA');
 ...
 assert.strictEqual(parse(status).mfa,true);
 assert.strictEqual(Object.prototype.hasOwnProperty.call(parse(status),'mfaRecovery'),false,'El estado de sesión no debe contener bypass MFA');
 ...
 assert(bridge.includes('if(this===window.localStorage&&key===KEY)return;'),'El puente debe bloquear persistencia del usuario interno en localStorage');
 const loginUi=fs.readFileSync('staff-login-ui.js','utf8');
 assert(!/localStorage\.setItem\(\s*KEY\b/.test(loginUi),'La UI de acceso interno no debe persistir datos o tokens de staff en localStorage');
 ...
 console.log('[test-admin-security] OK · MFA privilegiado obligatorio · bypass rechazado · cookie HttpOnly · auditoría íntegra');
