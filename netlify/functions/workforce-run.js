...
exports.handler=async event=>{
  ...
  try{
    if(event.httpMethod==='GET'){
      return json(200,{status:runtime.status(),families:governance.workforce.families()});
    }
    ...
    if(mode==='role'){
      ...
      await audit.append({event,actor:auth.email,action:'WORKFORCE_ROLE_RUN_COMPLETED',resource:record.id,outcome:'SUCCESS',metadata:{roleId:id,agentAction:action}}).catch(()=>{});
    ...
    if(mode==='family'){
      ...
      await audit.append({event,actor:auth.email,action:'WORKFORCE_FAMILY_RUN_COMPLETED',resource:String(body.family||''),outcome:'SUCCESS',metadata:{roles:result.roles,executed:result.executed,failed:result.failed}}).catch(()=>{});
      return json(200,{family:result});
    }
