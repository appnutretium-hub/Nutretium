...
function emailFrom(event){if(!secretConfigured())return null;const token=tokenFromHeader(event.headers||{});if(!token)return null;try{return verifyJWT(token).email||null}catch{return null}}
...
exports.handler=async function(event){
 ...
 if(event.httpMethod==='GET'){const email=emailFrom(event);if(!email)return json(401,{error:'Debes iniciar sesión.'});const [rows,pc]=await Promise.all([pedidosDe(email),pointsConfig()]);return json(200,{summary:resumenPedidos(rows,pc)})}
