...
async function upgradeHashIfNeeded(email,password,user,verification){if(!verification.needsRehash)return;const upgraded=hashPassword(password),at=new Date().toISOString();await usuarios.muta(email,current=>current.passwordHash===user.passwordHash?{...current,passwordHash:upgraded,passwordHashUpgradedAt:at,updatedAt:at}:null).catch(()=>null)}
exports.handler=async function(event){
 if(event.httpMethod==='OPTIONS')return{statusCode:204,headers:CORS,body:''};if(event.httpMethod!=='POST')return response(405,{error:'Method Not Allowed'});connectBlobs(event);if(!secretConfigured())return response(503,{error:'El registro y el inicio de sesión no están disponibles ahora mismo.'});
 ...
 if(body.action==='update'){let verified;try{verified=await session.verifyCustomerEventSession(event,{legacyToken:body.token,requireUser:false})}catch{return response(401,{error:'Sesión inválida, revocada o expirada.'})}if(!verified.user)return response(404,{error:'Usuario no encontrado.'});const ficha={name:String(body.name||'').trim(),surname:String(body.surname||'').trim(),phone:String(body.phone||'').trim()},problema=revisaFicha(ficha);if(problema)return response(400,{error:problema});let requestedDir=null;if(body.direccion!==undefined){requestedDir=direccion.normaliza(body.direccion);const problemaDir=direccion.revisa(requestedDir);if(problemaDir)return response(400,{error:problemaDir})}let actualizado;try{actualizado=await usuarios.muta(verified.email,current=>({...current,name:ficha.name,surname:ficha.surname,phone:ficha.phone,direccion:requestedDir||direccion.normaliza(current.direccion),updatedAt:new Date().toISOString()}))}catch{return response(409,{error:'Tu perfil cambió al mismo tiempo desde otra sesión. Inténtalo de nuevo.'})}if(!actualizado)return response(404,{error:'Usuario no encontrado.'});return profileResponse(actualizado,verified)}
