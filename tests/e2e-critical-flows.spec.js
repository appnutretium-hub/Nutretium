...
test('admin center: login interno no persiste JWT en localStorage',async({page})=>{
 ...
 await page.route('**/.netlify/functions/admin-session',route=>route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({ok:true})}));
 await page.route('**/.netlify/functions/admin-governance',route=>route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({actor:{email:'admin@nutretium.com',role:'admin'},security:{staffMfaRequired:true,mfaConfigured:true,audit:{valid:true,checked:1}},backups:[],tpvsol:{status:'NO VALIDADO',message:'Sin sincronización validada'}})}));
 ...
 await expect(page.locator('#app')).toBeVisible();
 const stored=await page.evaluate(()=>({length:localStorage.length,keys:Array.from({length:localStorage.length},(_,i)=>localStorage.key(i))}));
