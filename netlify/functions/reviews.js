 ...
 * Si llega una sesión válida, el servidor comprueba si ese cliente compró el
 * producto en un pedido PAID y marca la reseña como compra verificada.
 */
'use strict';
...
async function verifiedPurchase(token,product){
  ...
  let email;try{email=verifyJWT(token).email}catch{return false}
...
exports.handler=async function(event){
  ...
  if(event.httpMethod==='GET'){
    ...
    return{statusCode:200,headers:CORS,body:JSON.stringify({reviews:reviews.filter(r=>r.approved===true)})};
  }
  if(event.httpMethod==='POST'){
    ...
    if(!review.author||!review.product||!review.rating||!review.text)return{statusCode:400,headers:CORS,body:JSON.stringify({error:'Faltan campos obligatorios.'})};
