...
async function saveDraft(code,raw,actor){const checked=validate(code,raw);if(!checked.ok)throw Object.assign(new Error(checked.error),{statusCode:400});const s=store();if(!s)throw Object.assign(new Error('Contenido de producto no disponible.'),{statusCode:503});const previous=await read(code),next={...checked.data,status:'draft',createdAt:previous?.createdAt||new Date().toISOString(),updatedBy:String(actor||'').toLowerCase(),approvedAt:null,approvedBy:null};await s.setJSON(keyFor(code),next);return next}
async function approve(code,actor){const current=await read(code);if(!current)throw Object.assign(new Error('No existe una ficha avanzada para aprobar.'),{statusCode:404});const s=store();if(!s)throw Object.assign(new Error('Contenido de producto no disponible.'),{statusCode:503});const next={...current,status:'approved',approvedAt:new Date().toISOString(),approvedBy:String(actor||'').toLowerCase(),updatedAt:new Date().toISOString()};await s.setJSON(keyFor(code),next);return next}
...
async function replaceAll(rows=[]){const s=store();if(!s)throw new Error('Contenido de producto no disponible.');const current=await s.list({prefix:'product/'}).catch(()=>({blobs:[]}));for(const item of current.blobs||[])await s.delete(item.key).catch(()=>{});for(const row of rows){const checked=validate(row.code,row);if(!checked.ok)throw new Error(`Contenido inválido para ${row.code}: ${checked.error}`);const status=row.status==='approved'?'approved':'draft';await s.setJSON(keyFor(row.code),{...checked.data,status,createdAt:row.createdAt||new Date().toISOString(),updatedBy:row.updatedBy||'restore',approvedAt:status==='approved'?row.approvedAt||null:null,approvedBy:status==='approved'?row.approvedBy||null:null})}return rows.length}
module.exports={FIELDS,ARRAY_FIELDS,cleanCode,keyFor,validate,read,saveDraft,approve,list,replaceAll};