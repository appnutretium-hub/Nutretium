...
async function pointsConfig(){
 ...
 if(stored?.points?.managed===true){const n=Number(stored.points.perEuro)||0;return{enabled:Boolean(stored.points.enabled)&&n>0,perEuro:Boolean(stored.points.enabled)?Math.max(0,n):0,source:'managed'}}
 const explicitlyEnabled=String(process.env.NUTRETIUM_POINTS_ENABLED||'').trim().toLowerCase()==='true';
 const raw=String(process.env.NUTRETIUM_POINTS_PER_EURO||'').trim();const n=Number(raw);
 if(explicitlyEnabled&&raw&&Number.isFinite(n)&&n>0)return{enabled:true,perEuro:n,source:'env'};
 return{enabled:false,perEuro:0,source:'disabled'}
}
