...
(function(){
  ...
  const KEY='nutretium_user',MARKER=JSON.stringify({token:'http-only-cookie'});
  const nativeFetch=window.fetch.bind(window),nativeGet=Storage.prototype.getItem,nativeSet=Storage.prototype.setItem,nativeRemove=Storage.prototype.removeItem;
  ...
  Storage.prototype.removeItem=function(key){
    if(this===window.localStorage&&key===KEY){
      localLogout=true;
      nativeFetch('/.netlify/functions/admin-session',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'logout'}),keepalive:true}).catch(()=>{});
      return;
    }
    return nativeRemove.call(this,key);
  };
  window.fetch=async function(input,init){
    const url=typeof input==='string'?input:String(input?.url||''),response=await nativeFetch(input,init);
    ...
    const exchange=await nativeFetch('/.netlify/functions/admin-session',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({action:'exchange'})});
    if(exchange.ok){localLogout=false;return response}
