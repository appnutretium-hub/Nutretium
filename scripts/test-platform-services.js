'use strict';
const assert=require('assert');const hooks=require('../netlify/lib/webhooks');const worker=require('../netlify/functions/notification-worker')._test;const totp=require('../netlify/lib/totp');const guest=require('../netlify/functions/guest-order')._test;const refund=require('../netlify/functions/refund-redsys')._test;let n=0;function t(name,fn){fn();n++;console.log('✓',name)}
...
t('token invitado se compara constant-time',()=>{const h=guest.hash('abc');assert.strictEqual(guest.safeEq(h,guest.hash('abc')),true);assert.strictEqual(guest.safeEq(h,guest.hash('def')),false)});
